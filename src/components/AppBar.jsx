      ...
      <Container maxWidth="xl">
        <Toolbar disableGutters>
            ...
            >
              {pages.map((page) => (
                <MenuItem key={page.name} onClick={handleCloseNavMenu}>
                  <Link className="menu__link--small" to={page.path}>{page.name}</Link>
                </MenuItem>
              ))}
            </Menu>
          </Box>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages.map((page) => (
              <Link className="menu__link" key={page.name} to={page.path}>{page.name}</Link>
            ))}
          </Box>
          ...
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="Open settings">
              <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="/public/logo.svg" />
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: '45px' }}
